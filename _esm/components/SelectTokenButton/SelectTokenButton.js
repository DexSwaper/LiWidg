...
export const SelectTokenButton = ({ formType, compact }) => {
    ...
    const cardTitle = formType === 'from' && subvariant === 'custom'
        ? t(`header.payWith`)
        : t(`main.${formType}`);
    return (_jsx(SelectTokenCard, { component: "button", onClick: onClick, children: _jsxs(CardContent, { formType: formType, compact: compact, children: [_jsx(CardTitle, { children: cardTitle }), chainId && tokenAddress && (isChainLoading || isTokenLoading) ? (_jsx(SelectTokenCardHeader, { avatar: _jsx(AvatarBadgedSkeleton, {}), title: _jsx(Skeleton, { variant: "text", width: 64, height: 24 }), subheader: _jsx(Skeleton, { variant: "text", width: 72, height: 16 }), compact: compact })) : (_jsx(SelectTokenCardHeader, { avatar: isSelected ? (_jsx(TokenAvatar, { token: token, chain: chain })) : (_jsx(AvatarBadgedDefault, {})), title: isSelected ? token.symbol : defaultPlaceholder, titleTypographyProps: {
