...
import { ContentCopyRounded, DeleteOutline, MoreHoriz, OpenInNewRounded, TurnedInNot, Wallet, } from '@mui/icons-material';
...
export const RecentWalletsPage = () => {
    ...
    const handleViewOnExplorer = () => {
        if (selectedRecent) {
            const chain = getChainById(defaultChainIdsByType[selectedRecent.chainType]);
            window.open(`${chain?.metamask.blockExplorerUrls[0]}address/${selectedRecent.address}`, '_blank');
        }
        closeMenu();
    };
    ...
    return (_jsxs(SendToWalletPageContainer, { disableGutters: true, children: [_jsxs(ListContainer, { children: [recentWallets.map((recentWallet) => (_jsxs(ListItem, { sx: { position: 'relative' }, children: [_jsxs(ListItemButton, { disabled: requiredToChainType &&
                                    ...
                                    opacity: requiredToChainType &&
                                        ...
                                        ? 0.5
                                        : 1,
                                }, children: _jsx(MoreHoriz, { fontSize: "small" }) })] }, recentWallet.address))), !recentWallets.length && (_jsx(EmptyListIndicator, { icon: _jsx(Wallet, { sx: { fontSize: 48 } }), children: t('sendToWallet.noRecentWallets') })), _jsxs(Menu, { id: moreMenuId, elevation: 0, anchorOrigin: {
                            vertical: 'bottom',
                            horizontal: 'right',
                        }, transformOrigin: {
                            vertical: 'top',
